...
public_vacation_list = [
    '20170128', '20170129', '20170130', '20170131', '20170404',
    '20170414', '20170415', '20170417', '20170501', '20170503',
    '20170530', '20170701', '20171001', '20171002', '20171005',
    '20171028', '20171225', '20171226', '20180216', '20180217', 
    '20180218', '20180219', '20180330','20180331', '20180402', 
    '20180405', '20180501', '20180522','20180618', '20180702', 
    '20180925', '20181001', '20181017', '20181225','20181226',
]
...
training_data['date'] = pd.to_datetime(training_data['date'], format="%m/%d/%y")
...
training_data['is_rush_hour'] =training_data['time'].apply(lambda x: 0 if  0 <= x <= 6 or 20 <= x <= 23 else 1)
...
training_data['is_heavy_rainy_day'] = training_data['date'].apply(lambda x: 1 if x in heavy_rain_day else 0)
training_data['is_weekend'] = training_data['dayofweek'].apply(lambda x: 1 if x == 5 or x == 6 else 0)
training_data['is_free'] = (training_data['is_public_holiday'] + training_data['is_weekend']).apply(lambda x: 1 if x == 1 or x == 2 else 0)
training_data['is_weekday123'] = training_data['dayofweek'].apply(lambda x: 1 if x == 0 or x == 1 or x == 2 else 0)
training_data['is_weekday45'] = training_data['dayofweek'].apply(lambda x: 1 if x == 3 or x == 4 else 0)
...
testing_data['date'] = pd.to_datetime(testing_data['date'], format="%m/%d/%y")
...
testing_data['is_rush_hour'] = testing_data['time'].apply(lambda x: 0 if  0 <= x <= 6 or 20 <= x <= 23 else 1)
...
testing_data['is_heavy_rainy_day'] = testing_data['date'].apply(lambda x: 1 if x in heavy_rain_day else 0)
testing_data['is_weekend'] = testing_data['dayofweek'].apply(lambda x: 1 if x == 5 or x == 6 else 0)
testing_data['is_free'] = (testing_data['is_public_holiday'] + testing_data['is_weekend']).apply(lambda x: 1 if x == 1 or x == 2 else 0)
testing_data['is_weekday123'] = testing_data['dayofweek'].apply(lambda x: 1 if x == 0 or x == 1 or x == 2 else 0)
testing_data['is_weekday45'] = testing_data['dayofweek'].apply(lambda x: 1 if x == 3 or x == 4 else 0)
...
cv_params = {}
other_params = {'learning_rate': 0.06, 'n_estimators': 240, 'max_depth': 24, 'min_child_weight': 1, 'seed': 0,
               'subsample': 0.95, 'colsample_bytree': 0.93, 'gamma': 0.2, 'reg_alpha': 4, 'reg_lambda': 8}
